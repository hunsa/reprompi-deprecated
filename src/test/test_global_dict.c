...
int main(int argc, char* argv[]) {
  ...
  result = reprompib_init_dictionary(&params_dict, HASHTABLE_SIZE);
  if (result != 0) {
    printf("ERROR in hash table initialization");
    return 1;
  }
  for (i = 0; i < n; i++) {
    sprintf(buf, "%d", i);
  ...
  i = 5;
  sprintf(buf, "%d", i);
  result = reprompib_get_value_from_dict(&params_dict, buf, &s);
  if (result != 0 || s == NULL) {
    printf("get_value(%d) -> error\n", i);
  } else {
  ...
  printf("Current length is now %d (should be %d)\n", reprompib_dict_get_nkeys(&params_dict), n + n_elems);

  printf("Remove all elements...\n");
  n_elems = reprompib_dict_get_nkeys(&params_dict);
  reprompib_get_keys_from_dict(&params_dict, &key_list, &key_list_length);
  for (i = 0; i < n_elems; i++) {
    reprompib_remove_element_from_dict(&params_dict, key_list[i]);
  }
  printf("Current length is now %d (should be %d)\n", reprompib_dict_get_nkeys(&params_dict), 0);
