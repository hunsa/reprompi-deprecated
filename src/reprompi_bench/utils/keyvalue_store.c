...
void reprompib_init_dictionary(void) {
    ...
    dict.n_elems = 0;
    dict.data = (reprompib_params_keyval_t *) malloc(LEN_KEYVALUE_LIST_BATCH * sizeof(reprompib_params_keyval_t));
}
...
reprompib_dict_error_t reprompib_add_element_to_dict(char* key, const char* val) {
    ...
    if (old_val != NULL) {
        ...
        if (dict.n_elems == dict.size) {
            dict.size += LEN_KEYVALUE_LIST_BATCH;
            dict.data = (reprompib_params_keyval_t*) realloc(dict.data, dict.size * sizeof(reprompib_params_keyval_t));
        }
    }
    return ok;
}
