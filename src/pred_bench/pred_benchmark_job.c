...
void generate_pred_job_list(pred_options_t opts, pred_job_list_t* jlist) {
    ...
    if (opts.options.input_file != NULL) {
        read_input_pred_jobs(opts.options.input_file, jlist);
    } else {
        /* create job list according to command line options */

        if (opts.options.n_msize > 0 && opts.options.n_calls > 0) /* we have jobs to run */
        {
            // create the benchmark jobs
            jlist->jobs = (job_t*) malloc(
                    opts.options.n_calls * opts.options.n_msize * sizeof(job_t));
    ...
    if (jlist->n_jobs > 0) {
        ...
        if (my_rank == OUTPUT_ROOT_PROC) {
            ...
            if (opts.options.enable_job_shuffling > 0) {
