 ...
 *
 */
int read_input_pred_jobs(char* file_name, pred_job_list_t* jlist) {
  FILE* file;
  int result = 0;
...
void generate_pred_job_list(const nrep_pred_params_t* opts_p, const reprompib_common_options_t* common_opts_p,
  ...
  if (common_opts_p->input_file != NULL) {
    read_input_pred_jobs(common_opts_p->input_file, jlist);
  } else {
    /* create job list according to command line options */

    if (common_opts_p->n_msize > 0 && common_opts_p->n_calls > 0) /* we have jobs to run */
    {
      // create the benchmark jobs
      jlist->jobs = (job_t*) malloc(common_opts_p->n_calls * common_opts_p->n_msize * sizeof(job_t));

      i = 0;
      for (sizeindex = 0; sizeindex < common_opts_p->n_msize; sizeindex++) {
        for (cindex = 0; cindex < common_opts_p->n_calls; cindex++) {
          jlist->jobs[i].msize = common_opts_p->msize_list[sizeindex];
          jlist->jobs[i].call_index = common_opts_p->list_mpi_calls[cindex];
  ...
  MPI_Type_size(common_opts_p->datatype, &datatypesize);
  ...
  jlist->prediction_params = *opts_p;

  if (jlist->prediction_params.n_rep_min <= 0) {
    jlist->prediction_params.n_rep_min = 10;
  }

  if (jlist->prediction_params.n_rep_max < jlist->prediction_params.n_rep_min) {
    jlist->prediction_params.n_rep_max = 10;
  }

  if (jlist->prediction_params.n_rep_stride <= 0) {
    jlist->prediction_params.n_rep_stride = 10;
  }
  ...
  if (jlist->n_jobs > 0) {
    ...
    if (my_rank == OUTPUT_ROOT_PROC) {
      ...
      if (common_opts_p->enable_job_shuffling > 0) {
